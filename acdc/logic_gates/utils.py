...
def get_logic_gate_model(
    mode: Literal["OR", "AND"] = "OR", seq_len: Optional[int] = None, device="cuda"
) -> HookedTransformer:
    if seq_len is None:
        assert (
            1 <= seq_len <= MAX_LOGIC_GATE_SEQ_LEN
        ), "We need some bound on sequence length, but this can be increased if the variable at the top is increased"
    ...
    if mode == "AND":
        ...
        model.blocks[0].mlp.b_in[:] = (
            -(MAX_LOGIC_GATE_SEQ_LEN - 1) / MAX_LOGIC_GATE_SEQ_LEN
        )  # Unless everything in input is a 1, do not fire
...
def get_all_logic_gate_things(
    mode: str = "AND",
    device=None,
    seq_len: Optional[int] = 5,
    num_examples: Optional[int] = 10,
    return_one_element: bool = False,
) -> AllDataThings:
    assert mode == "OR"
